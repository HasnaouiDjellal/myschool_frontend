...
onMounted(async () => {
    ...
    const headers = { Authorization: `Bearer ${token}` }
    const res = await axios.get(`http://127.0.0.1:8000/api/levels/${props.id}/`, { headers })
    name.value = res.data.level_name
})


async function updatelevel() {
    await axios.patch(`http://127.0.0.1:8000/api/levels/${props.id}/`,
        { level_name: name.value },
        { headers }
    )
    emit('updated')
    emit('close')
...
<template>
    <div
        ...
        <div
            ...
            <h2 id="newClassTitle" class="self-stretch my-auto">Update level</h2>
            <!-- Close Button -->
        ...
        <div class="flex gap-2 items-center pl-4 mt-4 w-full max-w-sm">
            ...
            <input v-model="name" type="text" placeholder="Update Language"
        ...
        <div class="flex gap-2 items-center pl-4 mt-4 w-full max-w-sm text-base text-white whitespace-nowrap">
            ...
            <div class="flex gap-4 items-center ml-auto my-auto">
                <button @click="updatelevel"
                    class="gap-4 self-stretch px-4 py-2 my-auto rounded-lg bg-slate-600 min-h-[40px]"
